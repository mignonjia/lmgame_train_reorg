...
class EnvStateManager:
    ...
    def _init_env_instances(self, config):
        ...
        for tag, n_group in zip(config.env_configs.tags, config.env_configs.n_groups):
            ...
            done_groups += n_group
        # print(f"[DEBUG] split {self.mode}, env_list: {env_list}")
        return env_list
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        def _expand_seed(seed: int):
            ...
            else: # val, recall that for validation, we fix self.group_size=1
                seeds = []
                for n_group in self.config.env_configs.n_groups:
                    seeds.extend([[seed + i] for i in range(n_group)])
            return sum(seeds, [])
    ...
    def step(self, all_env_inputs: List[Dict], cur_turn: int):
        ...
        for env_input in all_env_inputs:
            ...
            if not turn_done and cur_turn < entry['max_turn']: # NOTE done environments are not sent for further llm generation (for efficiency)
