...
class VllmWrapperWg: # Thi is a developing class for eval and test
	...
	def generate_sequences(self, lm_inputs: DataProto):
		...
		# print(f'[DEBUG] input_texts: {input_texts}')
...
class LLMAgentProxy:
	...
	def rollout(self, dataproto: DataProto, val=False):
		...
		for cur_turn in range(self.config.agent_proxy.max_turn):
			...
			# lm_outputs.save_to_disk("lm_outputs.pkl")

			env_inputs: List[Dict] = ctx_manager.get_env_inputs(lm_outputs)
			env_outputs: List[Dict] = es_manager.step(env_inputs, cur_turn)
			# print(f"[DEBUG] cur_turn: {cur_turn}, env_outputs: {env_outputs}")
