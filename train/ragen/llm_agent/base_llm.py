    ...
    def __init__(self, provider: Union[str, LLMProvider], model_name: Optional[str] = None, 
        ...
        if isinstance(provider, LLMProvider):
            ...
            if provider.lower() == "openai":
                ...
                self.provider = OpenAIProvider(model_name or "gpt-4o", openai_api_key)
            elif provider.lower() == "anthropic":
                print(f'CLAUDE_API_KEY: {claude_api_key}')
                self.provider = AnthropicProvider(model_name or "claude-3-7-sonnet-20250219", claude_api_key)
            elif provider.lower() == "together":
                print(f'TOGETHER_API_KEY: {together_api_key}')
                self.provider = TogetherProvider(model_name or "meta-llama/Llama-3-70b-chat-hf", together_api_key)
            elif provider.lower() == "gemini":
                print(f'GEMINI_API_KEY: {gemini_api_key}')
                self.provider = GeminiProvider(model_name or "gemini-1.5-flash", gemini_api_key)
            else:
